...
class kohnint:
    def __init__(self, X_quad, X_bdr, layers1, layers2, theta, f, g1, g2, dudxq, dudyq, sigbdr):
        ...
        # loss
        # the weighing parameter * lebesgue measure should be 4 40 4e-5 80 for each term, common factor is factored out
        ...
        self.loss7 = 20 * tf.reduce_mean(tf.square(self.ab[:,0:1]-self.g1_tf)+tf.square(self.ab[:,1:2]-self.g2_tf))
        ...
        learning_rate = tf.train.exponential_decay(start_learning_rate, global_step, 2000, 0.7)
...
if __name__ == "__main__":
    ...
    # load data
    data = scipy.io.loadmat('YOUR_PATH/ntdiri1prj.mat')
    X_quad = data['X_quad']
    X_bdr = data['X_bdr']
    X_star = data['X_pred']
    f = data['f']
