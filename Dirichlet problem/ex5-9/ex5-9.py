...
class kohnint:
    def __init__(self, X_quad, X_bdr, layers1, layers2, theta, f, g1, g2, g3, dudxq, dudyq, dudzq, sigbdr):
        ...
        self.sig,self.sigx,self.sigy,self.sigz = self.net_sigma(self.xq_tf, self.yq_tf, self.zq_tf)
        
        self.abdr = self.net_a(self.xb_tf, self.yb_tf, self.zb_tf)
        self.diva = self.div1(self.xq_tf, self.yq_tf, self.zq_tf)
        self.sigb,_,_,_ = self.net_sigma(self.xb_tf, self.yb_tf, self.zb_tf)
        ...
        self.loss3 = 60 * tf.reduce_mean(
            tf.square(self.abdr[:, 0:1] - self.g1_tf) + tf.square(self.abdr[:, 1:2] - self.g2_tf) + tf.square(
                self.abdr[:, 2:3] - self.g3_tf))
        
        #self.loss4 = tf.reduce_mean(tf.square(self.sig))
        
        self.loss5 = tf.reduce_mean(tf.square(self.sigx) + tf.square(self.sigy) + tf.square(self.sigz))
    ...
    def train(self):
        ...
        tf_dict = {self.xq_tf: self.xq, self.yq_tf: self.yq, self.zq_tf: self.zq, self.xb_tf: self.xb,
                   self.yb_tf: self.yb, self.zb_tf: self.zb, self.f_tf: self.f, self.g1_tf: self.g1,
                   self.g2_tf: self.g2,self.g3_tf: self.g3, self.sigbdr_tf: self.sigbdr,
    ...
    def predict(self, X_star):
        sigma = self.sess.run(self.sig, {self.xq_tf: X_star[:, 0:1], self.yq_tf: X_star[:, 1:2], self.zq_tf: X_star[:, 2:3] })
        return sigma
...
if __name__ == "__main__":
    ...
    # load data
    data = scipy.io.loadmat('YOUR_PATH/ntdiri2prj.mat')
    X_quad = data['X_quad']
    X_bdr = data['X_bdr']
    X_star = data['X_pred']
    f = data['f']
    ...
    # model input
    model = kohnint(X_quad, X_bdr, layers1, layers2, theta, f, g1, g2, g3, dudxq, dudyq, dudzq, sigbdr)
    model.X_star = X_star
