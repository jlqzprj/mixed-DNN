...
class kohnint:
    def __init__(self, X_quad, X_bdr, layers1, layers2, theta, f, g1, g2, dudxq, dudyq, sigbdr):
        ...
        # (xq,yq) are the collocation points sampled from the domain interior Omega
        ...
        self.loss2 = 5 * tf.reduce_mean(tf.square(self.diva + self.f_tf))
        
        #self.loss3 = tf.reduce_mean(tf.square(self.sig))
        
        self.loss4 = tf.reduce_mean(tf.square(self.sigx) + tf.square(self.sigy))
        
        self.loss5 = 0.00001 * (self.loss4)
        
        self.loss6 = 1 * tf.reduce_mean(tf.square(self.sigb - self.sigbdr_tf))
        
        self.loss7 = 10 * tf.reduce_mean(
            tf.square(self.ab[:, 0:1] - self.g1_tf) + tf.square(self.ab[:, 1:2] - self.g2_tf)) #10=2.5*4
        
        # total variation term
        self.loss8 = 0.01 * tf.reduce_mean(self.siggrad)


        self.loss = self.loss1 + self.loss2 + self.loss5 + self.loss7 + self.loss8
    ...
    def train(self):
        ...
        tf_dict = {self.xq_tf: self.xq, self.yq_tf: self.yq, self.xb_tf: self.xb,
                   ...
                   self.dudyq_tf: self.dudyq, self.sigbdr_tf: self.sigbdr}
        ...
        for epoch in range(80000):
            ...
            iter_loss8 = self.sess.run(self.loss8, feed_dict=tf_dict)
            ...
            self.loss_vals8.append(1 * iter_loss8)
...
if __name__ == "__main__":
    ...
    # load data
    data = scipy.io.loadmat('YOUR_PATH/ntdiridisctn.mat')
    X_quad = data['X_quad']
    X_bdr = data['X_bdr']
    X_star = data['X_pred']
    f = data['f']
    ...
    # model input
    model = kohnint(X_quad, X_bdr, layers1, layers2, theta, f, g1, g2, dudxq, dudyq, sigbdr)
    model.X_star = X_star
    ...
    plt.semilogy(x1, model.loss_vals5, label='5')
    plt.semilogy(x1, model.loss_vals6, label='6')
    plt.semilogy(x1, model.loss_vals7, label='7')
    plt.semilogy(x1, model.loss_vals8, label='8')
