...
class kohnint:
    def __init__(self, X_quad, X_bdr, layers1, layers2, theta, normalx, normaly, normalz, f, g, dudxq, dudyq,
                 dudzq):
        
        self.theta = theta
        
        self.xq = X_quad[:, 0:1] # X_quad are the points sampled from domain interior
        self.yq = X_quad[:, 1:2]
        self.zq = X_quad[:, 2:3]
        ...
        # (xq,yq,zq) are the collocation points sampled from the domain Omega
        ...
        learning_rate = tf.train.exponential_decay(start_learning_rate, global_step, 2500, 0.7)
    ...
    def predict(self, X_star):
        sigma = self.sess.run(self.sig,
                              {self.xq_tf: X_star[:, 0:1], self.yq_tf: X_star[:, 1:2], self.zq_tf: X_star[:, 2:3]})
        return sigma
...
if __name__ == "__main__":
    ...
    # load data
    data = scipy.io.loadmat('YOUR_PATH/ntneu2prj.mat')
    X_quad = data['X_quad']
    X_bdr = data['X_bdr']
    X_star = data['X_pred']
    f = data['f']
    ...
    # model input
    model = kohnint(X_quad, X_bdr, layers1, layers2, theta, nx, ny, nz, f, g, dudxq, dudyq, dudzq)
    model.X_star = X_star
